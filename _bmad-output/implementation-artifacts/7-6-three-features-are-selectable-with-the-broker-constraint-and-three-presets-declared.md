...
4. **Given** a selection such as *API-only plus background task processing plus object storage*
...
- [ ] Task 1 — Declare the four features as independently selectable (AC: #1)
  - [ ] Complete `[features.<name>]` for all four in `accelerator.toml`, using the canonical names established in Story 7.1: `celery` (background task processing), `redis` (Redis cache), `ui` (server-rendered UI), `storage` (object storage).
  - [ ] Each feature carries `title` (the PRD's prose name, so the portal has a label), `packages`, the non-package surface keys, `constraints`, and nothing that makes it depend on a preset.
  - [ ] Assert "selected or absent, never present-and-disabled" as a testable property, not a slogan: no feature may be represented by a boolean *setting* the component reads at runtime. There is no `USE_CELERY`, no `ENABLE_UI`, no `settings.FEATURES` dict. A feature's presence is the presence of its paths and regions and nothing else. Add a gate test asserting no settings module defines a feature-toggle-shaped name.
  - [ ] Record in the carrier that the four selections are also the four pixi features Epic 8 turns into an `[environments]` matrix (AD-3), so the names must remain valid pixi feature names.
...
- [ ] Task 2 — Declare the broker constraint (AC: #2)
  ...
  - [ ] Record beside it what the constraint *is*: background task processing needs a broker, the broker is Redis, so background task processing without the Redis cache is not a valid pairing. Sixteen subsets of four features minus the four in which `celery` is selected and `redis` is not equals **twelve**.
  - [ ] Add a carrier-derived enumeration helper — `Carrier.valid_combinations() -> list[frozenset[str]]` in `tools/materializer/carrier.py` — that computes the space from `[features]` and `constraints`. Do not hardcode the number twelve or the twelve sets anywhere; both must fall out of the declaration, so adding a fifth feature or a second constraint does not require finding twelve literals.
...
- [ ] Task 3 — Declare the three presets (AC: #3, #4, #5)
  - [ ] `[presets.<name>]` for exactly three: `api-only`, `full-web-app`, `worker-enabled`. Each carries a `title` matching the PRD's prose name (*API-only*, *Full web app*, *Worker-enabled*), a `description`, and a `selects` list of feature names.
  - [ ] Suggested starting selections, to be confirmed against the PRD's own wording before writing: `api-only` selects nothing (or storage only — decide and record); `full-web-app` selects `ui` and `redis`; `worker-enabled` selects `celery` and `redis`. What matters for the ACs is not which subset each names but that each is a **starting point** and nothing else.
...
- [ ] Task 4 — Prove presets do not gate the selection space (AC: #4, #5)
  - [ ] Add a gate test asserting: for every one of the twelve valid combinations produced by `valid_combinations()`, materialization input can be constructed from the feature selections alone, with no preset named. This is AC #5, and it is the assertion that stops a preset from becoming a required field.
  - [ ] Add the AC #4 worked example as a named test case: *API-only plus background task processing plus object storage* — that is, `{celery, redis, storage}` (the broker constraint pulls `redis` in with `celery`, and "API-only" means the `ui` feature is absent). Assert it is a member of `valid_combinations()`. Record in the test's docstring that the AC's phrasing describes starting from a preset and editing, which is exactly what the preset must permit.
...
- [ ] Task 5 — Reconcile the feature space against `pixi.toml` (AC: #1, #2)
  ...
  - [ ] Assert the four canonical names are exactly `celery`, `redis`, `ui`, `storage` — no synonyms, no aliases, no display names leaking into the marker text. Story 7.2's markers, Story 7.4's UI move, Story 7.5's storage feature and Epic 8's pixi `[feature.<name>]` tables all key off these strings.
  - [ ] Do **not** create the `[environments]` matrix here. `pixi.toml:141-143` today declares only `default` and `dev`, both `solve-group = "default"`. Building the twelve pre-locked environments — and the shared `solve-group` that keeps `django-celery-beat`'s `django <6.1` cap from splitting the Django version across combinations — is Story 8.1.

- [ ] Task 6 — Tests (AC: all)
  - [ ] `tests/unit/materializer/test_combinations.py` (NEW): `valid_combinations()` returns twelve sets; every set containing `celery` also contains `redis`; the four `celery`-without-`redis` subsets are absent; the count is derived, not asserted against a literal list of twelve hand-written sets.
...
**FR-26 — the broker constraint.** *"The broker constraint is enforced at selection — twelve valid combinations, not sixteen."* Declared here; enforced by the materializer in Epic 8 as FR-34 (`epics.md:224`).

**FR-27 — presets pre-select without constraining.** *"Presets pre-select without constraining; every valid combination is reachable without a preset."* The failure this prevents is a preset list quietly becoming the menu — a lead developer who wants API-only plus background tasks plus object storage being told to pick one of three shapes.

**FR-24 — one carrier.** *"Four selectable features declared in one carrier with a single declared location and format, which is the only place a feature's extent is defined."* AD-1 forbids a second declaration site: the feature names, the constraint and the presets live in `accelerator.toml` and are read from there by everything.

**AD-3 — the features are native pixi features.** *"The four selectable features are declared as pixi features with an `[environments]` matrix, so one `pixi.lock` yields twelve pre-locked environments; combination *n*'s gate runs its materialized source under environment *n*. **All twelve environments share one `solve-group`**, without which `django-celery-beat`'s `django <6.1` cap makes the four Celery combinations resolve a different Django from the other eight and SC-1 stops meaning what it says."* That matrix is Story 8.1's work; this story only fixes the names it will use.
...
**FR-22 — the constraint is about deployment only.** *"The broker constraint is a statement about deployment only; all twelve combinations run locally with no broker."* The constraint governs which combinations exist, not how they run locally. Local development substitutes eager task execution (FR-18) in every combination that selected background task processing.
...
**Repository state, verified 2026-08-15.** `pixi.toml:141-143` declares exactly two environments, `default` and `dev`, both `solve-group = "default"`. There are no `[feature.<name>]` tables for `celery`, `redis`, `ui` or `storage` — the only pixi feature that exists is `dev` (`[feature.dev.dependencies]` `:106-132`, `[feature.dev.tasks]` `:184-187`, `[feature.dev.activation.env]` `:152-156`). The twelve-combination matrix does not exist. The `django-celery-beat` comment at `pixi.toml:33-34` already records the `django <6.1` cap that makes AD-3's shared solve-group load-bearing.

**The twelve combinations, for reference.** With `ui` and `storage` free (4 pairings) and the `{celery, redis}` axis restricted to `{}`, `{redis}`, `{celery, redis}` (3 states, the fourth — `{celery}` — excluded), the space is 4 × 3 = 12. The dev agent must not hardcode this; it must fall out of `valid_combinations()`.
...
- The combination count must be asserted as a derived property (`len(valid_combinations()) == 12` given the declared features and constraints), plus a structural assertion that no valid combination contains `celery` without `redis`. Do not write a test that compares against twelve literal frozensets — it would pass while `valid_combinations()` is a hardcoded list, which is the implementation this story forbids.
...
- The names `celery`, `redis`, `ui`, `storage` are load-bearing across five places: `[features.<name>]` in `accelerator.toml`, `feature:<name>` dispositions, `feature:<name>` / `/feature:<name>` markers in four `core` files, pixi `[feature.<name>]` tables (Epic 8), and the `[environments]` matrix names (Epic 8). Fixing them here, with a two-way reconciliation test, is what stops a rename in Epic 8 from being a silent partial rename.
- The PRD's prose names — background task processing, Redis cache, server-rendered UI, object storage — are the `title` fields. The portal order surface will show titles and send names; the field list for that surface is an open item owned by the portal team, and until it exists the fixture set covers the AD-25 parameters and the four feature booleans (spine Open Items; Story 8.6).
...
- [Source: _bmad-output/planning-artifacts/epics.md#Story 8.1] — the twelve pre-locked environments
