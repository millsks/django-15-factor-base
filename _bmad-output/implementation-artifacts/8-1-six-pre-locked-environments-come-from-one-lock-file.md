...
1. **Given** the three selectable features
...
- [ ] Task 1: Move feature-owned runtime packages out of `[dependencies]` into `[feature.<name>.dependencies]` in `pixi.toml` (AC: #1)
  ...
  - [ ] Leave `crispy-bootstrap5` (`:18`) and `django-crispy-forms` (`:41`) in `[dependencies]` with its conda-forge cap comment. The server-rendered interface is immovable core (AD-29), not a selectable feature: `templates/allauth/elements/field.html` and `fields.html` render the FR-4 interactive sign-in flow through `crispy`, so the form-styling packages are present in every combination and there is no `[feature.ui.dependencies]` table to create.
  - [ ] Leave `opentelemetry-instrumentation-django`, `-asgi` and `-psycopg`, the API, SDK and OTLP exporter in `[dependencies]` — Story 7.2 requires them present in all six.

- [ ] Task 2: Declare the six-environment matrix (AC: #1, #2)
  - [ ] Extend `[environments]` (`pixi.toml:141-143`) with six entries, each `features = [...]` naming the dev feature plus its selected feature subset, and each carrying `solve-group = "default"` — the same group `default` and `dev` already use.
  - [ ] Name each environment `combo-<id>` where `<id>` is the selected feature names sorted alphabetically and joined with `-`, or `none` when nothing is selected. The six: `combo-none`, `combo-redis`, `combo-storage`, `combo-redis-storage`, `combo-celery-redis`, `combo-celery-redis-storage`.
...
- [ ] Task 3: Assert the solve group holds (AC: #2, #3)
  - [ ] Add `tests/unit/test_environment_matrix.py` parsing `pixi.toml` with `tomllib` and asserting: exactly six `combo-*` environments exist; the six names equal the enumerated set; every one declares `solve-group = "default"`.
...
- [ ] Task 4: Assert environments are not fat (AC: #4)
  - [ ] In `tests/integration/test_environment_resolution.py`, assert `celery` and `django-celery-beat` appear in the two `combo-celery-*` environments and in no other; `django-redis`, `redis-py`, `hiredis` in exactly the four `redis`-selecting environments; `opentelemetry-instrumentation-celery` and `-redis` in exactly their feature's environments.
  - [ ] Assert `opentelemetry-instrumentation-django`, `-asgi`, `-psycopg`, `opentelemetry-api`, `opentelemetry-sdk`, `opentelemetry-exporter-otlp-proto-http`, `django`, `psycopg`, `djangorestframework`, `crispy-bootstrap5` and `django-crispy-forms` appear in all six.
...
- **AD-3** (binding): "The three selectable features — background task processing, Redis and object storage — are declared as pixi features with an `[environments]` matrix, so one `pixi.lock` yields six pre-locked environments; combination *n*'s gate runs its materialized source under environment *n*. **All six environments share one `solve-group`**, without which `django-celery-beat`'s `django <6.1` cap makes the two Celery combinations resolve a different Django from the other four and SC-1 stops meaning what it says." *Prevents:* "six independent dependency solves; a combination passing its gate in an environment fat enough to hide an import it should not have; six combinations silently testing two different Django versions."
- **AD-29 / revision 3**: the server-rendered interface mechanism is `core`, not a selectable feature. There is no `feature:ui`, no `[feature.ui.dependencies]`, and no `src/features/` — **AD-33 is retired**. The three remaining features own regions of `core` paths (AD-24) and dependency entries, and nothing else.
- **What "environment *n*"** means for the gate: the reference workspace's pre-locked environment supplies the packages; the materialized tree supplies the source. Story 8.8 wires that; this story only has to make the six environments exist and be pre-locked from one lock.
- **AD-24**: `pixi.toml` is one of the declared region-bearing `core` paths — the set is open and carrier-declared, so this story asserts nothing about how many such paths there are. This story adds feature dependency tables; Story 8.3 prunes them by declared marker. Do **not** introduce any other sub-file removal mechanism — not conditional imports, not settings-module inheritance, not `try/except ImportError`.
...
- **Broker constraint** (FR-26, declared in Story 7.6): background task processing requires the Redis cache. Six valid combinations, not eight. Enforcement in the materializer is Story 8.5; here it only shapes which six environments exist.
...
|---|---|---|
| `pixi.toml` | UPDATE | Today: `[dependencies]` (`:14-81`) holds every runtime package including celery, redis and crispy; `[environments]` (`:141-143`) declares only `default = { solve-group = "default" }` and `dev = { features = ["dev"], solve-group = "default" }`. This story moves the three features' packages into `[feature.<name>.dependencies]` — the crispy packages stay in `[dependencies]`, being core — and adds six `combo-*` environments, all `solve-group = "default"`. Preserve: the `[workspace]` block, `[target.linux-64.dependencies]` / `[target.osx-arm64.dependencies]` gunicorn+uvicorn-worker scoping (`:85-91`), `[pypi-dependencies]` / `[pypi-options]` (`:98-104`), `[activation.env] COVERAGE_CORE` (`:145-150`), every task table and every rationale comment. |
| `pixi.lock` | UPDATE | Regenerated by `pixi install` after the matrix lands. Never hand-edited. |
| `tests/unit/test_environment_matrix.py` | NEW | Parses `pixi.toml`; asserts the six environment names, the shared solve-group, and the broker constraint over the matrix. |
| `tests/integration/test_environment_resolution.py` | NEW | Parses `pixi.lock`; asserts one Django across all six and per-feature package containment. |
...
Variance from the seed: the seed shows `src/config/authorization/`, `src/config/startup/`, `src/django_apps/`, `accelerator.toml`, `component.toml`, `Dockerfile`, `tools/materializer/` and `tools/harness/` — none exist today. Earlier epics create them; nothing here depends on them.
...
- Assertions the ACs demand, stated exactly: six `combo-*` environments; all six `solve-group = "default"`; one identical `django` version across all six in `pixi.lock`; no `celery`-selecting environment lacking `redis`; feature packages confined to their feature's environments.
...
- [Source: _bmad-output/planning-artifacts/architecture/architecture-django-15-factor-base-2026-08-15/ARCHITECTURE-SPINE.md#AD-29] — the interface mechanism is core; no `feature:ui`
- [Source: _bmad-output/planning-artifacts/architecture/architecture-django-15-factor-base-2026-08-15/ARCHITECTURE-SPINE.md#Revision 3 — the interface mechanism becomes core] — three features, six valid combinations, six pre-locked environments
- [Source: _bmad-output/planning-artifacts/epics.md#Story 7.2] — instrumentation package placement across the six
- [Source: _bmad-output/planning-artifacts/epics.md#Story 7.6] — the broker constraint and the count of six
