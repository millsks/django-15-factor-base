...
1. **Given** five declaration sites exist today
...
4. **Given** `uvicorn --app-dir` accepts a single directory
   **When** roots are declared
   **Then** it is never used as a declaration mechanism

## Tasks / Subtasks

- [ ] Task 1 — Convert the retained site to a `sources` remapping first (AC: #2, #3)
...
- [ ] Task 2 — Remove the four fallback declaration sites (AC: #1, #4)
  ...
  - [ ] `pixi.toml:186` — **a sixth site AD-7 does not name.** The `serve-reload` task reads `uvicorn config.asgi:application --app-dir src --reload --reload-dir src`. Remove `--app-dir src` here too; AC #4 says `--app-dir` "is never used as a declaration mechanism", which admits no per-task exception. **Keep `--reload-dir src`** — that is a file-watch target, not an import-root declaration, and removing it would break autoreload.
...
- [ ] Task 3 — Resolve the `"."` pythonpath entry deliberately (AC: #1, #3)
  - [ ] AC #1 says "the pytest `pythonpath` setting ... [is] removed", and AD-7 names `pyproject.toml` `[tool.pytest.ini_options] pythonpath` as one of the five sites. The `"."` entry is not a *source* root — it makes `tests.factories` importable, and `tests/` is a package (`tests/__init__.py` exists). `tests/conftest.py` does `from tests.factories import UserFactory`; `tests/unit/` and `tests/integration/` also carry `__init__.py`. `pyproject.toml:145` sets `--import-mode=importlib`.
...
- [ ] Task 4 — Prove the three runtimes agree (AC: #3)
...
`epics.md:345`: "Story 1.6's single import-root site gains the `src/django_apps/` root in Epic 9 without gaining a second site." Epic 9 Story 9.2 adds `src/django_apps/` to the same `sources` table. Write the declaration and its comment so that addition is a one-line edit in one place.
...
| --- | --- | --- |
| `manage.py:24-26` | `manage.py:23-25` (comment at `:22`) | **Drifted by one line** — the insert is `:23-25`. |
| `asgi.py:18-20` | `src/config/asgi.py:18-20` (comment at `:17`) | Holds exactly. |
| `wsgi.py` (no range) | `src/config/wsgi.py:24-26` (comment at `:23`) | Holds; range recorded. |
| `pyproject.toml [tool.pytest.ini_options] pythonpath` | `pyproject.toml:149`, value `[ "src", "." ]` | Holds; the `"."` entry is not a source root — see Task 3. |
| `--app-dir src` in the `serve` task | `pixi.toml:179` | Holds. **Also present at `pixi.toml:186` in `serve-reload`** — a sixth site AD-7 does not name. |
| Retained: `[tool.hatch.build.targets.wheel]` `sources` remapping | `pyproject.toml:126-127` uses `packages`, not `sources` | **Does not hold yet** — the retained site must itself be converted. |
...
- [Source: _bmad-output/planning-artifacts/epics.md:146] — AD-7 restated in the epic preamble with the five named sites.
- [Source: _bmad-output/planning-artifacts/epics.md:345] — the single site gains `src/django_apps/` in Epic 9 without gaining a second site.
- [Source: _bmad-output/planning-artifacts/epics.md:235] — collapsing the five sites is a precondition for Epic 4's allowlist.
