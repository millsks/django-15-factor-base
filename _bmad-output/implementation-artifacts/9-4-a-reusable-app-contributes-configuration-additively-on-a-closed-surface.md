...
- [ ] Task 4 — Read the component's selected features (AC: #8)
  ...
  - [ ] The list carries **three** feature names — `celery`, `redis`, `storage`. There is no `ui`: revision 3 makes the interface mechanism `core`. The reference application selects all three; the materializer (Epic 8) writes each combination's actual three booleans into its materialized tree, across the six valid combinations.
...
- [ ] Task 5a — Stage 2: every registered navigation URL name resolves (AC: #9)
  - [ ] In the stage-2 module of `src/config/startup/` (Story 4.3), iterate the composed navigation registry and `django.urls.reverse` — or `get_resolver().reverse_dict` — each entry's URL name. A name that does not resolve raises `ImproperlyConfigured` naming the contributing application, the label and the URL name.
...
- [ ] Task 7 — Tests (AC: all)
  ...
  - [ ] `tests/unit/test_contributable_surface.py`: the carrier enumeration equals the runtime constant, with `src/config/startup/` authoritative (AC #7); the allowlist and the contributable surface are one module-level declaration, asserted by identity of the object, not by comparing two copies (AC #7); no entry in `CONTRIBUTABLE_KEYS` is a namespace, prefix pattern or wildcard (AC #5); the four global keys are in `REFUSED_GLOBAL_KEYS`, and the navigation registry is in `CONTRIBUTABLE_SEQUENCES` while `MIDDLEWARE` and `AUTHENTICATION_BACKENDS` are in neither (AC #5, #6).
...
| `src/config/startup/surface.py` | UPDATE | **Does not exist today**; `src/config/startup/` is created by Story 4.1 and the FR-17 allowlist by Story 4.6. Extend that same declaration with `CONTRIBUTABLE_KEYS`, `CONTRIBUTABLE_SEQUENCES` (three names, navigation registry included), `REFUSED_GLOBAL_KEYS`, `KEY_FEATURE_OWNER`. This is the **authoritative** copy of the contributable surface (AD-26); `accelerator.toml` mirrors it. |
| `src/config/startup/` stage-2 module | UPDATE | Created by Story 4.3. Add the AC #9 refusal: every registered navigation URL name resolves in the URLconf, or `ImproperlyConfigured`. |
...
| `component.toml` | UPDATE | **Does not exist today**; created by **Story 5.1**, which also declares `[features] selected` (AD-28) and the adopted-application list. This story reads both; it introduces neither. |
...
- [Source: _bmad-output/planning-artifacts/epics.md#Story 4.3] — the stage-2 module the AC #9 refusal joins
