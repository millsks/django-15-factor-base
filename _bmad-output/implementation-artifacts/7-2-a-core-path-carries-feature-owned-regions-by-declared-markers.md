...
- [ ] Task 2 — Mark the region-bearing paths (AC: #1, #2, #5)
  ...
  - [ ] **`:296-335` is the extent as the block stands today, and Story 6.1 moves it.** That story relocates `DJANGO_STRUCTLOG_CELERY_ENABLED` and its two comment lines from `base.py:289-291` into the top of the Celery block, precisely so this story can enclose the whole thing in one marker pair — which shifts the block's start and its end by the number of lines moved. Read the block's actual boundaries out of the file when placing the markers, and record the **post-move** range in the carrier. Story 6.1 states the new extent in its Completion Notes; prefer the file over either number.
