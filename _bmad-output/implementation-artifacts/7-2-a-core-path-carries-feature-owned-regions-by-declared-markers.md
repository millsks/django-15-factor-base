...
2. **Given** the three known region-bearing paths
...
- [ ] Task 1 — Define the marker syntax and the `[regions]` declaration shape (AC: #1, #3)
  - [ ] In `accelerator.toml`, populate `[[regions]]` as an array of tables. Each entry: `path` (repo-relative), `feature` (one of `celery`, `redis`, `ui`, `storage`), and a short `reason`. Add nothing else — the marker pair in the file is the extent; the carrier does not duplicate line numbers, which would drift on every edit.
  - [ ] Marker form: an open line `feature:<name>` and a close line `/feature:<name>`, each written as a **line comment in the file's own comment syntax**. Python: `# feature:celery` / `# /feature:celery`. TOML: `# feature:celery` / `# /feature:celery`. Django templates, if a region ever lands in one: `{# feature:ui #}` / `{# /feature:ui #}`.
...
- [ ] Task 2 — Mark the region-bearing paths (AC: #1, #2, #5)
  - [ ] `src/config/settings/base.py` — wrap the Celery settings block. The epic and AD-24 cite `:296-313`; **the range has drifted in extent** (see Source Tree below). The block begins at the `# Celery` header on `:296` and ends at `CELERY_WORKER_HIJACK_ROOT_LOGGER = False` on `:335`, immediately before the `# django-allauth` header on `:336`. Open marker above `:296`, close marker after `:335`. Record the corrected extent in the carrier `reason` field.
  - [ ] `src/config/settings/base.py` — wrap `REDIS_URL` / `REDIS_SSL` at `:293-294` as `feature:redis`. These are read by the Celery block (`CELERY_BROKER_URL`, `:302`) and by `production.py`'s `CACHES` (`:36`), and both consumers exist only where `redis` is selected — `celery` requires `redis` (FR-26), so `redis` is present whenever either is. This region is **not named in the epic**; declare it and record why.
  - [ ] `src/config/settings/base.py` — wrap the feature entries in the installed-app lists individually: `"django_celery_beat"` at `:110` as `feature:celery`; `"crispy_forms"` at `:105` and `"crispy_bootstrap5"` at `:106` as `feature:ui`. One marker pair per contiguous run, not one pair around `THIRD_PARTY_APPS`.
  - [ ] `src/config/observability/telemetry.py` — the epic cites the per-instrumentor calls at `:134-137`; **the range holds exactly**. But only two of the four calls are feature-owned: `:134` `DjangoInstrumentor()` and `:136` `PsycopgInstrumentor()` are `core` (present in all twelve), `:135` `CeleryInstrumentor()` is `feature:celery`, `:137` `RedisInstrumentor()` is `feature:redis`. That is **two separate single-line regions**, not one region spanning `:134-137`. Mark each individually.
  ...
  - [ ] `pixi.toml` — wrap the feature-owned dependency lines. `feature:celery`: `celery` (`:17`), the `django-celery-beat` entry and its explanatory comment block plus its three explicit dependencies (`:22-39`), `opentelemetry-instrumentation-celery` (`:67`). `feature:redis`: `django-redis` (`:43`), `hiredis` (`:46`), `redis-py` (`:48`), `opentelemetry-instrumentation-redis` (`:69`). `feature:ui`: `crispy-bootstrap5` (`:18`), `django-crispy-forms` (`:41`). Keep each entry's rationale comment inside its region — the rationale travels or is pruned with what it explains.
  - [ ] `pixi.toml` — leave `opentelemetry-api`, `opentelemetry-sdk`, `opentelemetry-exporter-otlp-proto-http`, `opentelemetry-instrumentation-django`, `opentelemetry-instrumentation-asgi` and `opentelemetry-instrumentation-psycopg` (`:58-61`, `:66`, `:68`) unmarked and therefore `core` — present in all twelve (AC #5, second clause).
...
- [ ] Task 3 — Build the region parser and the two-way region reconciler (AC: #3)
  ...
  - [ ] Also fail on a marker present in a file that carries no `[[regions]]` declaration at all — otherwise a marker added to a fourth file passes reconciliation because nothing declares that file.
...
**AD-29 — no `feature:*` inside `src/django_service/`.** No marker in this story goes inside `src/django_service/`. UI surface leaving that package is Story 7.4's move, and it moves paths, not regions.
...
| `src/config/settings/base.py` | UPDATE | **Today:** the single composed settings module — `BASE_DIR`/`APPS_DIR` (`:15-18`), `DATABASES` with a sqlite fallback (`:57-80`), `DJANGO_APPS`/`THIRD_PARTY_APPS`/`LOCAL_APPS` (`:93-123`), structlog wiring (`:281-291`), `REDIS_URL`/`REDIS_SSL` (`:293-294`), the Celery block (`:296-335`), allauth (`:336-352`), DRF (`:357-364`), spectacular (`:373-379`). **This story adds:** four marker pairs — `feature:ui` around `:105-106`, `feature:celery` around `:110`, `feature:redis` around `:293-294`, `feature:celery` around `:296-335`. **Preserve:** every setting value, every existing comment, the `# ruff: noqa: ERA001, E501` header at `:1`, and the module's ordering — Epic 4 will append the stage-1 refusal call as the last statement, so do not add anything after the final block. |
...
- `src/config/observability/telemetry.py:134-137` — **holds exactly.** `:134` `DjangoInstrumentor().instrument()`, `:135` `CeleryInstrumentor().instrument()`, `:136` `PsycopgInstrumentor().instrument()`, `:137` `RedisInstrumentor().instrument()`. Correction to the epic's framing: this is not one four-line region. Two of the four calls are `core`; the feature-owned regions are the single lines `:135` and `:137`, plus their imports at `:21` and `:24`.
- `src/config/settings/base.py:296-313` — **the start holds; the extent has drifted.** `:296` is the `# Celery` header and `:298` the first setting, so `:296` is correct as the open. But `:313` (`CELERY_RESULT_BACKEND_ALWAYS_RETRY = True`) sits mid-block: the Celery settings continue through `CELERY_RESULT_BACKEND_MAX_RETRIES` (`:315`), `CELERY_ACCEPT_CONTENT` (`:317`), the serializers (`:319`, `:321`), the time limits (`:324`, `:327`), `CELERY_BEAT_SCHEDULER` (`:329`), `CELERY_WORKER_SEND_TASK_EVENTS` (`:331`), `CELERY_TASK_SEND_SENT_EVENT` (`:333`) and `CELERY_WORKER_HIJACK_ROOT_LOGGER` (`:335`). The `# django-allauth` header follows at `:336`. **Use `:296-335`.** Closing at `:313` would leave twenty-two Celery settings in eight combinations — harmless as dead configuration, fatal as an orphan the coverage signal cannot see and as a `CELERY_BEAT_SCHEDULER` referencing `django_celery_beat`, a package those combinations do not have.

**Two region-bearing surfaces the epic does not name — declare them and record why:**

1. `src/config/settings/base.py:293-294` — `REDIS_URL` and `REDIS_SSL`. `feature:redis`. Consumed by the Celery block (`:302`, `:304`, `:306`, `:308`) and by `production.py` (`:12`, `:36`).
2. `src/config/settings/production.py:31-44` — the `CACHES` block configuring `django_redis.cache.RedisCache`, and its `from .base import REDIS_URL` at `:12`. This is `feature:redis` surface in a **fourth** `core` path. `CACHES` is not defined in `base.py` at all; the deployed Redis cache lives only here, and the in-process substitute only in `local.py:18-26`. Mark the `production.py` block in this story and record the fourth path in the carrier. Note that Epic 3 restructures the local substitutions and Epic 4 adds the FR-14 feature-scoped refusals as further `feature:redis` / `feature:celery` regions — the carrier must accommodate more than three region-bearing paths by construction, not by exception.

Also present and relevant but **not this story's**: `src/config/settings/local.py:75-80` (`CELERY_TASK_ALWAYS_EAGER` / `CELERY_TASK_EAGER_PROPAGATES`) is `feature:celery` region surface; Epic 3 rewrites the local settings module and Story 4.4 makes the eager-execution refusal a declared region. If `local.py` still carries that block unmarked when this story runs, mark it and declare it.
...
- No file moves and no new source packages under `src/`. This story adds comments to three existing `core` paths plus `production.py`, and one new machinery module.
- The Structural Seed lists exactly three region-bearing paths. The tree carries a fourth (`production.py`) and a fifth arrives with Epic 3's `local.py` rewrite. Model `[[regions]]` as an open array of tables, never as three fixed keys, and never hard-code the three filenames in the reconciler.
...
- Repository, verified 2026-08-15: `src/config/settings/base.py:105,106,110,293-294,296-335`; `src/config/observability/telemetry.py:21,24,134-137`; `src/config/settings/production.py:12,31-44`; `src/config/settings/local.py:18-26,75-80`; `pixi.toml:17,18,22-39,41,43,46,48,58-61,66-69`
