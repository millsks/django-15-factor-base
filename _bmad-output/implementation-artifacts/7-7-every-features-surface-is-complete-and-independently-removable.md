...
- [ ] Task 2 — Make each feature's removal a single declared query (AC: #1)
  ...
  - [ ] Repeat the same completeness pass for `redis`, `ui` and `storage`, and record each feature's full surface in the carrier so a reader can answer the removal question from the declaration alone.
