...
- [ ] Task 1 — Create `accelerator.toml` at the repository root with the full top-level table skeleton (AC: #1, #2)
  ...
  - [ ] Use the four canonical feature names from the AD-4 dependency graph: `celery` (background task processing), `redis` (Redis cache), `ui` (server-rendered UI), `storage` (object storage). These strings are the `<name>` in `feature:<name>` dispositions, in the AD-24 markers, and in the pixi `[feature.<name>]` tables Epic 8 builds. Do not invent alternative spellings.
  ...
  - [ ] Under `[verification]`, declare `pinned_subset = []` with a comment recording that AD-19's pinned all-pairs subset is populated when the twelve-combination matrix exists (Epic 8). An empty list is a declared-but-unpopulated slot, not an omission.
...
- [ ] Task 2 — Declare each feature's package and non-package surface (AC: #2)
  - [ ] For each of the four features write `[features.<name>]` with: `title` (the PRD's prose name), `packages` (the conda-forge package names the feature adds), `paths` (its owned path list — may be empty at this story; Stories 7.4/7.5/7.7 fill them), `constraints` (Story 7.6 populates `celery.requires = ["redis"]`), and `presets` back-reference handled in `[presets]`.
  ...
  - [ ] `features.ui.packages` = `django-crispy-forms`, `crispy-bootstrap5`.
  ...
  - [ ] Enumerate the non-package surface categories explicitly as separate keys so nothing is inferred: `settings_fragments`, `app_modules`, `observability`, `templates`, `static`, `tests`. An empty list is a legitimate declaration; an absent key is not.
...
- [ ] Task 3 — Declare a disposition for every tracked path in the reference application (AC: #3, #4, #6)
  ...
  - [ ] `machinery`: `accelerator.toml` itself (AC #1), `tools/**` (the materializer, Epic 8), `Dockerfile` (AD-15, Epic 5), `_bmad/**`, `_bmad-output/**`, `.agents/**`, `.claude/**`, `.bmad-loop/**`, `.github/agents/**`, `.github/copilot/**`. Record explicitly that `machinery` is also the default, so these entries are documentation of intent rather than load-bearing — except where an entry prevents a *claim* elsewhere.
  - [ ] Split `.github/` and `docs/` per FR-37: workflows that gate the *component* travel as `core`; workflows that gate the *accelerator's own machinery* (SonarCloud for this repository, the twelve-combination matrix, release automation) are `machinery`. Decide each of `.github/workflows/{ci,labeler,release,sonarqube,stale}.yml` individually and record the reason inline. Same for `docs/`: component-facing documentation travels (NFR-8), accelerator-facing does not.
  - [ ] `tenant`: `src/django_apps/**`.
  - [ ] `feature:<name>`: leave empty at this story where a later story owns the enumeration (7.4 for `ui`, 7.5 for `storage`, 7.7 for the full sweep). Every path must still be claimed by *something* for AC #5 to pass — until 7.4/7.5/7.7 land, UI and storage surface is claimed as `core` and 7.4/7.7 re-disposition it. State that transition in a comment so the intermediate state is not mistaken for the final one.
...
- The Structural Seed places `accelerator.toml` and `component.toml` at the root and `tools/materializer/` beside `src/` and `tests/`. This story creates the first and third; `component.toml` is Epic 5's.
