...
- [ ] Task 2 — Turn the swallowed failure into a logged event (AC: #1, #2)
  ...
  - [ ] Place the two new settings **adjacent to the `CACHES` block**, contiguously, so the whole Redis-owned settings surface in `production.py` can later be enclosed in one `feature:redis` marker pair. Do not scatter them.
...
- **AD-24** — the three declared region-bearing `core` paths are `src/config/settings/base.py`, `src/config/observability/telemetry.py` (per-instrumentor calls at `:134-137` — **verified: the range still holds exactly**) and `pixi.toml`. `src/config/settings/production.py` is **not** among them. Note the consequence honestly: this story adds Redis-feature-scoped settings to a file the architecture has not yet declared as region-bearing. Keeping them contiguous with the `CACHES` block is what lets Epic 7 declare a region there without re-cutting anything. Do **not** achieve feature scoping with a conditional import, a settings-module override, or a `try/except ImportError` — AD-24 forbids all three, and it forbids them everywhere, not only in the three named files.
...
No layout change; this story stays inside `src/config/settings/` and the mirrored test tree. The variance to record for Epic 7: the Redis feature's settings extent is split across `src/config/settings/production.py` (the `CACHES` block, now plus two logging flags) and `src/config/settings/base.py:293-294` (`REDIS_URL`/`REDIS_SSL`, which the Celery block also consumes). AD-24 names only `base.py`, `telemetry.py` and `pixi.toml` as region-bearing, so `production.py` will need either a fourth region-bearing entry or a different treatment when the Redis feature is declared. Flag it; do not solve it here.
...
- [Source: _bmad-output/planning-artifacts/architecture/architecture-django-15-factor-base-2026-08-15/ARCHITECTURE-SPINE.md#AD-24] — the three region-bearing paths and the forbidden sub-file mechanisms.
