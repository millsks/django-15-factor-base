...
description: 'Trace changed behavior to the places that observe it and report where verification would not catch a regression. Orthogonal to adversarial and edge-case review - it audits the tests, not the code. Use when you need to know whether a change is actually protected.'
---
...
- **also_consider** (optional) — Areas to keep in mind alongside normal verification-gap analysis
...
Unlike the other hunters, this review reads the repository: tracing a consumer and reading its tests is the method. Do not assign severity labels, rankings, or priority levels.
...
- If content is empty, or cannot be decoded as text, return `[{"location":"N/A","trigger_condition":"Input empty or undecodable","guard_snippet":"Provide valid content to review","potential_consequence":"Review skipped — no analysis performed","gap_shape":"other"}]` and stop

### Step 2: Screen for behavioral change

If the change is non-behavioral, stop here and return zero findings (`[]`). Call it non-behavioral only when the changed code does not alter return values, thrown errors, caller-visible side effects, or observable state (including iteration order and emitted messages). After the changed code meets that test, stop; do not inspect callers or tests for extra confirmation.

Common non-behavioral examples: formatting, comments, whitespace; pure renames; trivial getters/setters and pass-throughs; type-only or compiler-enforced changes with no runtime effect.

### Step 3: Find the behavior that changed

Identify what behavior changed compared to the previous version: output, side effect, branch, error path, schema/event shape, config default, validation/authorization rule, external contract. If the change affects more than one behavior, handle each separately.

Treat broad-impact changes as behavioral even when no single changed line looks important: dependency, toolchain, build/config, and data-file changes all qualify.

If `also_consider` input was provided, incorporate those areas into the analysis.

### Step 4: Trace where that behavior is used
...
Follow a path only while the changed behavior is reachable and unverified. Stop when a test at that boundary would fail, the consumer does not observe the changed behavior, or the next hop is guesswork (dynamic dispatch, reflection, outside-repo consumers). Prefer the nearest observable boundary, often one to three hops away, especially across contract, integration, or service edges. If there are more than five similar consumers, group obvious repeats and check representative paths; expand only when a consumer observes the behavior differently.

### Step 5: Qualify the consumer, then check its test

For each consumer, name the smallest realistic regression this consumer would observe: invert the branch, drop the default, omit the field, return the old error code, skip the integration call. This is the Demonstration. If no such regression exists, drop the path; untested downstream code is not a finding.

A `missing-adoption-gap` qualifies not by the adoption failure alone but by a supersession signal: the change gives clear evidence the new behavior is meant to replace the local one — spec intent, naming or docs, a replaced sibling site, deleted duplicate logic, or a test defining the new rule — and the local site shares the same observable contract. Without a supersession signal and a shared observable contract, it is a refactor suggestion, not a verification-gap finding. Once both hold, check whether any test for that site would flag the non-adoption; missing coverage of the non-adoption is the gap itself, not a disqualifier.
...
### Step 6: Confirm each finding is real
...
Return ONLY a valid JSON array of objects. Each finding contains the four canonical fields plus this review's extras:

```json
[{
  "location": "the changed surface: the exact behavior or contract that changed, file:line",
  "trigger_condition": "the gap, in one line (max 15 words)",
  "guard_snippet": "the missing assertion or check, fit to the repo's own way of verifying (single-line escaped string, no raw newlines or unescaped quotes)",
  "potential_consequence": "the concrete thing that ships wrong and why the tests checked would not fail (max 15 words)",
  "gap_shape": "regression-gap | missing-adoption-gap | broken-verification-gap | other",
  "consumer": "the impacted consumer or site, named concretely with file:line",
  "evidence": "what you actually checked: what the relevant test asserts with file:line, or the symbol/import-reference searches run and their result"
}]
```

For `gap_shape: "other"` findings the four canonical fields suffice; `consumer` and `evidence` are optional.

No extra text, no explanations, no markdown wrapping. An empty array `[]` is valid — and is the required output when the change is non-behavioral or every changed behavior is verified.
